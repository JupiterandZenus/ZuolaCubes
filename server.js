...
async function startServer() {
    ...
    setInterval(async () => {
        if (!DISCORD_WEBHOOK_URL) {
            console.log('⚠️ Discord webhook URL not configured - skipping hourly status update');
            return;
        }

        try {
            console.log('📊 Sending hourly system status update to Discord...');
            
            // Collect current system statistics
            const statistics = await collectSystemStatistics();
            
            if (statistics) {
                const uptimeHours = Math.floor(statistics.server.uptime / 3600);
                const uptimeMinutes = Math.floor((statistics.server.uptime % 3600) / 60);
                const memoryUsageMB = Math.round(statistics.server.memory.heapUsed / 1024 / 1024);
                const memoryTotalMB = Math.round(statistics.server.memory.heapTotal / 1024 / 1024);

                await sendDiscordNotification(
                    "📊 Farm Manager - Hourly Status",
                    `System status report for ${new Date().toLocaleString()}\n\n` +
                    `🤖 **Agents:** ${statistics.agents.online}/${statistics.agents.total} online\n` +
                    `📋 **Accounts:** ${statistics.accounts.total} total, ${statistics.accounts.completed_tasks} completed\n` +
                    `💾 **Memory:** ${memoryUsageMB}MB / ${memoryTotalMB}MB\n` +
                    `⏱️ **Uptime:** ${uptimeHours}h ${uptimeMinutes}m\n` +
                    `🔌 **Connections:** ${statistics.server.connected_clients} WebSocket clients\n` +
                    `🌐 **Server:** http://localhost:${PORT}/`,
                    0x3498db, // Blue color for status updates
                    "📊 Status Report"
                );
                console.log('✅ Hourly status update sent to Discord');
            }
        } catch (error) {
